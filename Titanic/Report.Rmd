...
Below is the proportion of correct responses in the train set using a decision tree on Sex+Pclass+SibSp+projAge.

```{r,echo=FALSE}
    tree <- rpart("Survived~Sex+Pclass+SibSp+projAge",data=train)
    trainTree <- mutate(train,rawTree=predict(tree))
    trainTree <- mutate(trainTree,projSurvived=ifelse(trainTree$rawTree>.5,1,0))
    sum(trainTree$Survived==trainTree$projSurvived)/length(trainTree$Survived)
```
