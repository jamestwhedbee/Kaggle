...
ggplot(store,aes(Assortment,StoreType))+geom_point(position="jitter")
...
* Promo2 at time of sale?
* Promo2Interval started within one month?
* Competition open at time of sale?
* Competition distance quantile, if open
...
ggplot(rossman,aes(Week,Sales))+geom_point(position="jitter",aes(col=Year),alpha=.1)
...
salesByStore <- aggregate(Sales ~ StoreType,rossman,mean)
...
* Most stores are closed on day 7, day 1 have best sales
* Stores with closer competition have more sales (presumably busier areas)
* Stores with promos have more sales
* Stores with assortment b or type b have more sales
* Sales are higher on school holidays, most stores closed on state holidays
* Christmastime has best sales
* Sales have increased every year

Is it likely that best predictor is just previous weeks sales at same store, while avoiding state or school holidays? Maybe just impute values using k nearest neighbors?
